...
def request_help(new_request: schemas.HelpDesk, db: Session = Depends(get_db), current_user: str = Depends(oauth.get_current_user)):
    request = ClientHelpRequest(date_posted = datetime.now(), owner_customer_no = current_user.customer_no, id = uuid4(), **new_request.dict())
    try:
        db.add(request)
        db.commit()
        db.refresh(request)
...
def report_problem(new_request: schemas.Report, db: Session = Depends(get_db), current_user: str = Depends(oauth.get_current_user)):
    request = ClientReports(date_posted = datetime.now(), owner_customer_no = current_user.customer_no, id = uuid4(), **new_request.dict())
    try:
        print(request)
        db.add(request)
        db.commit()
